...
if __name__ == "__main__":
   ...
   word_vec_reshape = tf.reshape(word_vec, [batch_size, 78, embed_size])
   ...
   for i in range(0,len(test_data),batch_size):
      if len(test_data[i:i+batch_size]) == batch_size:
         ...
         ylist.extend(sess.run(tf.argmax(yreshape,2), feed_dict={x: test_data[i:i+batch_size], y_: test_chunk[i:i+batch_size], l: test_sent_len[i:i+batch_size]}))
         y_list.extend(sess.run(tf.argmax(y_,2),feed_dict={x: test_data[i:i+batch_size], y_: test_chunk[i:i+batch_size], l: test_sent_len[i:i+batch_size]}))
